...
- when consider change/anything on database, read existing migration, dont update/edit existing migration, instead check what already applied create new changes based on existing applied pattern, minimizing data broken/failed adjustment, while preserving existing data (if any).
